...
class UsuarioWeb
{
	...
	*/
	public static function getByUsername($p_db, $p_username) {
		...
		$str_sql =
			"  SELECT id_usuario_web, nombre_usuario, contrasena, nombre, apellidos, email, 0+activo AS activo, 0+borrado AS borrado" .
		 	"  FROM usuario_web u" .
	...
	public static function seekSpecial($p_db, $p_param) {
		...
		$str_sql =
			"  SELECT id_usuario_web AS id, nombre_usuario, contrasena, nombre, apellidos, email, 0+activo AS activo, 0+borrado AS borrado" .
		 	"  FROM usuario_web u" .
    ...
    public static function seek($db, $parameters, $order, $direction, $offset, $limit) {
		...
		try {
			...
			$str_sql =
			"  SELECT u.id_usuario_web AS id, u.nombre_usuario, u.contrasena, u.nombre, u.apellidos, u.email, 0+u.activo AS activo, 0+u.borrado AS borrado" .
		 	"  FROM usuario_web u";
